	...
	for _, file := range files {
		...
		*/
		if strings.Contains(file.Name(), ".yml") {
			file_name := strings.Replace(file.Name(), ".yml", "", -1)
			...
			*/
			var table_count int
			query := fmt.Sprintf("SELECT CAST(COUNT(TABLE_NAME) AS UNSIGNED) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA='%s' AND TABLE_NAME='%s'", connection_configs.Get("name"), file_name)
			db.QueryRow(query).Scan(&table_count)
			if table_count == 0 {
				logger.Println("red", "[TABLE]: ", "SKIPPING [" + file_name + "]")
			}

			if table_count == 1 {

				/*
					Seems like we have a table for the yaml file that contain
